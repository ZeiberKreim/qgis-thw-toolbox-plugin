...
class SelectionTool(QgsMapTool):
    ...
    def _paint_overlay(self, event):
        ...
        for handle in self.resize_handles:
            painter.drawEllipse(
                handle.x() - self.handle_size, handle.y() - self.handle_size, self.handle_size * 2, self.handle_size * 2
            )
    ...
    def _calculate_selection_bounds(self):
        ...
        feature_size = (
            self.selected_feature["size"] if "size" in [field.name() for field in self.layer.fields()] else 30.0
        )
        ...
        self.selection_bounds = QRect(
            int(pixel_point.x() - half_size), int(pixel_point.y() - half_size), int(size_in_pixels), int(size_in_pixels)
        )
